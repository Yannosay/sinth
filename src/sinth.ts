...
function compileExprToJS(expr: Expression, loopVars?: Set<string>): string {
  switch (expr.kind) {
    case "literal":
      ...
      return "null";
    case "variable":
      if (loopVars && expr.name && loopVars.has(expr.name)) {
...
function registerExpr(ctx: CompileCtx, expr: Expression): number {
...
function renderAttr(attr: Attr, paramMap: Map<string, string>, ctx?: CompileCtx): string {
